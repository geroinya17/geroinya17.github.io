...
;( function( $, window, undefined ) {
	...
	// Настройки
	$.DLMenu.defaults = {
		// Классы для эффектов анимации
		animationClasses : { in : 'dl-animate-in-1', out : 'dl-animate-out-1' },
		// Обратный вызов: щелчок по пунку выпадающего меню
		// el - это элемент выпадающего меню (li); имя содержит уровень выпадения
		onLevelClick : function( el, name ) { return false; },
		// Обратный вызов: если щелчок мыши пришёлся не на элемент LI выпадающего списка
		// el - это элемент выпадающего меню (li); ev - это объект (obj)
	...
	$.DLMenu.prototype = {
		_init : function( options ) {
			...
			// Кэшируем некоторые элементы и значения
			this._config();
			...
			// Анимация и название событий
			this.animEndEventName = animEndEventNames[ Modernizr.prefixed( 'animation' ) ] + '.dlmenu';
			// Конечный переход и название события
			this.transEndEventName = transEndEventNames[ Modernizr.prefixed( 'transition' ) ] + '.dlmenu',
			// Поддерживает ли браузер анимаю и переходы
		...
		_initEvents : function() {
			...
			this.$trigger.on( 'click.dlmenu', function() {
				...
				if( self.open ) {
					...
					self._openMenu();
					// Щелчок не на меню, закрывает его
		...
		// сброс меню в исходное состояние (видим лишь первый элемент)
		_resetMenu : function() {
